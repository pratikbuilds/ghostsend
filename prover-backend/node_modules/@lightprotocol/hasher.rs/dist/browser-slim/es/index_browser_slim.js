 ...
 * @returns {Uint8Array}
 */
function blake2$1(input, hash_length) {
    const ptr0 = passArray8ToWasm0$1(input, wasm$1.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN$1;
    const ret = wasm$1.blake2(ptr0, len0, hash_length);
    var v2 = getArrayU8FromWasm0$1(ret[0], ret[1]).slice();
    wasm$1.__wbindgen_free(ret[0], ret[1] * 1, 1);
    return v2;
}
...
function poseidon$1(inputs) {
    const ret = wasm$1.poseidon(inputs);
    if (ret[2]) {
...
async function __wbg_init$1(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('light_wasm_hasher_bg.wasm', import.meta.url);
    }
 ...
 * @returns {Uint8Array}
 */
function blake2str(input, hash_length) {
    const ptr0 = passStringToWasm0(input, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.blake2str(ptr0, len0, hash_length);
 ...
 * @returns {Uint8Array}
 */
function blake2(input, hash_length) {
    const ptr0 = passArray8ToWasm0(input, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.blake2(ptr0, len0, hash_length);
    var v2 = getArrayU8FromWasm0(ret[0], ret[1]).slice();
    wasm.__wbindgen_free(ret[0], ret[1] * 1, 1);
    return v2;
}
...
function poseidon(inputs) {
    const ret = wasm.poseidon(inputs);
    if (ret[2]) {
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('hasher_wasm_simd_bg.wasm', import.meta.url);
    }
...
function stringify(input) {
    if (input.length > 0 && input[0] instanceof BN) {
        return input.map((item) => item.toString(10));
    }
    else {
        return input;
    }
...
class WasmFactory {
    ...
    static async loadHasher(options) {
...
function wasmHasher(hasher) {
    const WasmFactory = class {
        blakeHash(input, hashLength) {
        ...
        poseidonHashBN(input) {
            return new BN(this.poseidonHash(input));
        }
        poseidonHashString(input) {
...
const loadWasmSimd = async (module) => {
    if (simdMemory === undefined) {
        simdMemory = __wbg_init(module ?? wasmSimdInit?.()).then((x) => {
            return wasmHasher({
                blake2str: blake2str,
                blake2: blake2,
                poseidon: poseidon,
...
const loadWasm = async (module) => {
    if (sisdMemory === undefined) {
        sisdMemory = __wbg_init$1(module ?? wasmInit?.()).then((x) => {
            return wasmHasher({
                blake2str: blake2str$1,
                blake2: blake2$1,
                poseidon: poseidon$1,
            });
        });
...
let simdEnabled;
const hasSimd = () => simdEnabled ??
    (simdEnabled = WebAssembly.validate(new Uint8Array([
        0, 97, 115, 109, 1, 0, 0, 0, 1, 5, 1, 96, 0, 1, 123, 3, 2, 1, 0, 10, 10,
        1, 8, 0, 65, 0, 253, 15, 253, 98, 11,
    ])));

export { WasmFactory, hasSimd as hasWasmSimd };
